...
static class XMLTools
{
    ...
    {
        bool canParse = Enum.TryParse<T>((string?)element.Element(name), out var result);
        return  (canParse)?  (T?)result : null;
    }
    public static T? ToEnumNullable222<T>(this XElement element, string name) where T : struct, Enum=>
   ...
   public  static  XElement CreateElement<T>(this T entity) where T: struct 
    {
        // var res= (entity.GetType().GetProperties().ToList().Select (a => new XElement(a.Name,a.GetValue(entity ) ))); ))
        //  return res;
